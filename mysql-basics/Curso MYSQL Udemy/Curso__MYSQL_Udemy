...
/* Com o uso do AS, podemos mudar o nome do campo que será projetada*/
...
select departamento, sexo, count(*) from funcionarios where sexo = 'Feminino' group by departamento order by departamento; /*Para saber quantas mulheres tem em cada departamento por ordem alfabetica de departamento*/

select departamento, sexo, count(*) from funcionarios where sexo = 'Feminino' group by departamento order by count(*); /*Para saber quantas mulheres tem em cada departamento por ordem do menor para maior*/

select departamento, sexo, count(*) from funcionarios where sexo ='Masculino' group by departamento order by departamento; /*Para saber quantos homens tem em cada departamento por ordem alfabetica de departamento*/

select departamento, sexo, count(*) from funcionarios where sexo = 'Masculino' group by departamento order by count(*); /*Para saber quantas homens tem em cada departamento por ordem do menor para maior*/
...
/*Quando trabalhamos com AND, o menor vai na frente para não degradar o banco*/

SELECT * FROM FUNCIONARIOS
WHERE
( DEPARTAMENTO = 'Lar' AND SEXO = 'Feminino' )
OR
( DEPARTAMENTO = 'Filmes' AND SEXO = 'Feminino' ); /*Maneira que não degrada o banco*/

select * from funcionarios where sexo = 'Feminino' AND (departamento = 'Filmes' or departamento = 'Lar'); /*outra possibilidade*/
...
SELECT NOME, SEXO, ENDERECO
FROM CLIENTE
WHERE EMAIL = NULL; /*Náo funciona, pois não pode usar = Null*/

SELECT NOME, SEXO, ENDERECO
FROM CLIENTE
WHERE EMAIL IS NULL; /*forma correta*/

SELECT NOME, SEXO, ENDERECO
FROM CLIENTE
WHERE EMAIL IS NOT NULL; /*forma correta*/
...
UPDATE CLIENTE
SET EMAIL = 'LILIAN@HOTMAIL.COM'; /*Errado, pois assim, ele atualizou todos os emails de todos os registros e não s[o da Lilian. Por isso devemos usar o Where*/
...
/* DELETANDO REGISTROS COM A CLAUSULA DELETE. Passo a passo para não destrutir os registros.*/

DELETE FROM CLIENTE; /* NÃO FACA ISSO*/

/*Melhor maneira - Passo a Passo*/

SELECT * FROM CLIENTE WHERE NOME = 'ANA'; /* Primeiro: Faça um select da onde vocë quer deletar*/

SELECT COUNT(*) FROM CLIENTE; -- TOTAL 6 /*Segundo: Conte os registros da tabela da onde você quer deletar*/

SELECT COUNT(*) FROM CLIENTE -- TOTAL DELETADO 1 /* Terceiro: Faça um count da onde vocë quer deletar*/
WHERE NOME = 'ANA';

DELETE FROM CLIENTE
WHERE NOME = 'ANA'; /* Quarto: Uso  odelete com where*/

SELECT COUNT(*) FROM CLIENTE; -- RESTANTE 5 /* Quinto: Faça a conta com outro select e count na tabela e veja se deletou*/
...
DELETE FROM CLIENTE
WHERE NOME = 'CARLA'
OR EMAIL = 'LILIAN@HOTMAIL.COM'; /*Errado! Com OR, ele deleta todos que tem ou e-mail igual a esse ou com nome carla*/
...
GROUP BY ENDERECO; /*Não irá agrupar por quem mora no RJ, pois o endereço está vetorizado, ou seja, ele não agrupa só pq no endereco tem RJ. O endereco possui bairros e ruas diferentes apesar de serem do RJ*/
...
'RUA COPPER LEAF - WILLIAMSBURG - KITCHENER'); /* Iremos inserir um novo telefone para a Carla*/
...
[KA, FIESTA, UNO, CIVIC]-> CARROS OBS: Exemplo de campo vetorizado -> Tabela Cliente possui telefone -> dividimos em uma tabela cliente e criamos outra tabela telefone

2 - TODO CAMPO MULTIVALORADO SE TORNAR'A OUTRA TABELA.
QUANDO O CAMPO FOR DIVISÍVEL OBS: Exemplo de campo Multivalorado -> Rua Abacaxi - Cidade: SP - É Multivalorado pois possui duas informacoes...a rua e a cidade. Deve ser separato em outra tabela
...
A CHAVE PRIMARIA É A "NUMERACAO DOS REGISTROS". GERALMENTE EM ORDEM DE 1 ATÉ N COM ID.

Com essas 3 regras, iremos remodelar nosso banco. Iremos criar 3 tabelas: Endereco, Cliente e telefone 
todas possuindo uma ID.(Ver dpcumento Word Criado na pasta)

SENDO:

TABELA ENDERECO: IDENDERECO, RUA, BAIRRO, CIDADE, ESTADO
TABELA TELEFONE: IDTELEFONE, NUMERO, TIPO
TABELA CLIENTE:  IDCLIENTE, NOME,SEXO,CPF,EMAIL

Exercicio: ENDERECO: É OBRIGATORIO
           TELEFONE: O CLIENTE NAO É OBRIGATORIO INFORMAR

ESQUEMA MODELO

+---------------------+  Não é Obrigatorio(0) e o cliente pode ter quantos telefones quiser (n)
| TELEFONE            |  Portanto Relação (0:n)
+---------------------+  
| IDTELEFONE          |  
| NUMERO              |_______________(0:n)____________
| TIPO                |                                |
+---------------------+                                |
                                                       | Juntando a relação 0:n do telefone com 1:1 do cliente, o segundo numero do par é a relação entre um e outro
                                                       | Logo, a relação Cliente x Telefone é (1:n) -> Traduzindo: 1 cliente pode ter n telefones
                                                       |
+---------------------+                                |
| CLIENTE             |<_____________(1:1)_____________|                                
+---------------------+  Ao cadastrar o telefone é obrigatorio entrar com o cliente, portanto 1                               
| IDCLIENTE           |  Qual é maximo de clientes que eu posso ter por telefone? 1   Portanto, relação (1:1) 
| NOME                |
| SEXO                | Ao cadastrar o endereço é obrigatorio entrar com o cliente, portanto 1
| CPF                 | Qual é maximo de clientes que eu posso ter para esse endereco? 1   Portanto, relação (1:1)                            
| EMAIL               | <______________________________                              
+---------------------+              (1:1)             |
                                                       |
                                                       |
                                                       |
+---------------------+                                |  Juntando a relação 1:1 do endereço com 1:1 do cliente, o segundo numero do par é a relação entre um e outro
| ENDERECO            |                                |  Logo, a relação Cliente x Endereço é (1:1) -> Traduzindo: 1 cliente pode ter 1 endereço
+---------------------+                                |
| IDENDERECO          |                                |
| RUA                 |                                |
| BAIRRO              | ____________(1:1)______________|
| CIDADE              |
| ESTADO              |   É Obrigatorio(1) e o cliente só pode ter 1 endereço(1)
+---------------------+          Portanto Relação (1:1)

*/

/* CADINALIDADE */

/*VER DIAGRAMA ENTIDADE E RELACIONAMENTO DESENHADO. Exercicio Comercio*/

/*

EXERCICIO: AO REALIZAR O CADASTRO DO CLIENTE TEMOS AS SEGUINTES CONSIDERAÇOES:
...
/*Vamos criar um novo banco com as condições anteriores*/
...
--Primary key unica na tabela e não pode se repetir. Comando Auto_increment, o banco auto incrementa, não precisando gerar toda vez que criar.
--NOT NULL: Não pode ser um valor nulo, é obrigatorio ser preenchido. Imagina o usuario cadastrando um cliente e não colocar o nome. O sistema irá informar que é obrigatorio
--ENUM é dominio. Imagine uma caixa onde o usuario só pode colocar M ou F. O Dominio é M ou F. Em outros bancos, isso se chama Constrant de Check
--UNIQUE: Não pode ter duas pessoas com o mesmo e-mail
--UNIQUE: Não pode ter duas pessoas com o mesmo cpf
...
CREATE TABLE ENDERECO(
	...
	ID_CLIENTE INT UNIQUE, -- RELACIONAMENTO 1:1 -> POR ISSO É UNIQUE...SÓ PODE TER UM ENDERECO

	FOREIGN KEY(ID_CLIENTE)
	REFERENCES CLIENTE(IDCLIENTE)
);

-- A FOREIGN KEYS É A CHAVE ESTRANGEIRA QUE SE RELACIONA COM A CHAVE PRIMARIA DA OUTRA TABELA. GERALMENTE, A CHAVE ESTRANGEIRA É NOMEADA COM UNDERLINE. NO CASO ACIMA, A CHAVE PRIMARIA DA TABELA CLIENTE IRÁ VIRAR A CHAVE ESTRANGEIRA DA TABELA ENDERECO

CREATE TABLE TELEFONE(
	IDTELEFONE INT PRIMARY KEY AUTO_INCREMENT, 
	TIPO ENUM('RES','COM','CEL') NOT NULL,
	NUMERO VARCHAR(10) NOT NULL,
	ID_CLIENTE INT, -- RELACIONAMENTO 0:N -> O CLIENTE PODE TER VARIOS TELEFONES, POR ISSO NÃO TEM A PALAVRA UNIQUE

	FOREIGN KEY(ID_CLIENTE)
	REFERENCES CLIENTE(IDCLIENTE)
);

-- A FOREIGN KEYS É A CHAVE ESTRANGEIRA QUE SE RELACIONA COM A CHAVE PRIMARIA DA OUTRA TABELA. GERALMENTE, A CHAVE ESTRANGEIRA É NOMEADA COM UNDERLINE. NO CASO ACIMA, A CHAVE PRIMARIA DA TABELA CLIENTE IRÁ VIRAR A CHAVE ESTRANGEIRA DA TABELA TELEFONE
...
/* EM RELACIONAMENTO 1 X 1 A CHAVE ESTRANGEIRA FICA NA TABELA MAIS FRACA */ --  A tabela mais fraca é o endereco
...
/*
+------------+--------------------+----------+----------------+--------+------------+ - NO RELACIONAMENTO 1:1, A CHAVE ESTRANGEIRA NÃO SE REPETE E O NUMERO DE REGISTROS EM CLIENTES E ENDERECO É IGUAL
...
+------------+------+----------+------------+ NO RELACIONAMENTO 0:N, A CHAVE ESTRANGEIRA SE REPETE E O NUMERO DE REGISTROS EM CLIENTES E TELEFONE NÃO É IGUAL.
...
/*Não é o ideal, pois where não é usado para juncao e sim para selecao, como explicado acima*/
...
/*Esse é o ideal, pois você colocou o que você quer projetar (NOME,SEXO,BAIRRO,CIDADE), a origem da tabela (CLIENTE, pois primeiro vem nome e sexo na selecao(ideal)), a juncao(ENDERECO), da onde ira juntar (ON) e a selecao(WHERE SEXO = F) */

/*A TABELA QUE TEM MAIS LIGACOES, É A QUE VAI NO FROM. NO CASO, A TABELA CLINETE É LIGADA A MAIS 2 TABELAS, ENDERECO E TELEFONE, LOGO, ELA VAI NO FROM */

/* O PRIMEIRO TERMO DO ON, É A TABELA DO QUE ESTA NO FROM. LOGO, AO ESCREVER ON, PRIMEIRO VAI O ID DA TABELA CLIENTE POR ELA ESTA NO FROM*/
...
/*Voce tem que colocar os pontos para cada coluna e para cada ID, pois se não, ID_CLIENTE da tabela endereco e ID_CLIENTE da telefone da ambiguidade */
...
/*Voce pode colocar apelido nas tabelas (FROM TABELA ESPACO 'APELIDO')*/
...
/* 
  DDL - DATA DEFINITION LANGUAGE
*/
/*DEFINICAO DE VARIAVEIS É COMANDO DDL*/
...
/* 6 12 13 14 18 19 -> ALGUNS CLIENTES ESTÃO COM NOME FEMININO E CAMPO SEXO COMO MASCULINO. POR ISSO VAMOS ATUALIZAR
PARA ISSO, PRIMEIRAMENTE FAZEMOS UM SELECT COM DOS CAMPOS QUE QUEREMOS ATUALIZAR */ 
...
/*PARA NÃO PRECISAR COLOCAR UM MONTE DE OR, PODEMOS USAR O IN*/

SELECT * FROM CLIENTE
WHERE IDCLIENTE IN (6,12,13,14,18,19);
...
/*FACA A QUERY POR PARTES E VÁ TESTANDO*/

/*Primeiro, vamos fazer os Inner Join*/
...
/*Segundo, vamos restringir com o Sexo. Somente Feminino agora*/
...
/*Terceiro, vamos restringir mais ainda. Vamos colocar a cidade Rio de Janeiro e o Bairro no filtro where*/
...
/*Quarto, vamos restringir mais ainda. Vamos colocar o tipo de telefone como residencial ou comercial no filtro where*/
...
/*Esse Or induz ao erro, pois ele vai puxar pelo Or. Devemos usar o parenteses*/
...
/*AO REALIZAR ESSA PROJECAO, TEMOS PESSOAS COM EMAIL NULL. PARA ISSO, PODEMOS CRIAR UMA FUNCAO PARA NÃO MOSTRAR NULL. ESSA FUNCAO É A IF NULL*/
...
/*APLIQUE UMA FUNCAO, NA COLUNA QUE ESTA COM OS DADOS NULOS AO FAZER A QUERY. NO CASO, O DADO DE ENTRADA DELA, É A COLUNA A TER DADO NULO*/
...
/*VIEW SÃO PONTEIROS QUE APONTA PARA A QUERYS. SERIA COMO SE FOSSE UMA FUNCAO */
...
/* AO CRIAR A VIEW, VOCÊ PODE CHAMAR ELA*/
...
/* AS VIEWS SÃO MOSTRADAS NO COMANDO SHOW TABLES, POIS ELAS SÃO COMO SE FOSSE UMA TABELA VIRTUAL*/
...
/*PODEMOS FAZER UMA QUERY DENTRO DA VIEW. NO CASO ACIMA, FIZEMOS UMA PROJECÃO DA QUERY VIEW, FAZENDO UM SELECT DELE, SOMENTE DO NOME,NUMERO E ENDERECO*/
...
/*NÃO É POSSIVEL INSERIR VALORES E DELETAR VALORES EM VIEWS COM JOIN*/
...
/*NESSE CASO FOI POSSIVEL DELETAR, POIS ESSA VIEW CRIADA NÃO POSSUI JOIN*/

INSERT INTO V_JOGADORES VALUES('GUERRERO','RS');

/*NESSE CASO FOI POSSIVEL INSERIR O VALOR, POIS ESSA VIEW CRIADA NÃO POSSUI JOIN*/
...
/*É PERMITIDO TAMBEM SELECIONAR(FILTRAR) QUALQUER COISA NA VIEW*/
...
/*ORDENANDO POR 1, É A COLUNA 1 DA TABELA (INDICE), NO CASO, A TABELA NUMERO*/

SELECT * FROM ALUNOS
ORDER BY 2;

/*ORDENANDO POR 2, É A COLUNA 2 DA TABELA (INDICE), NO CASO, A TABELA NUMERO*/
...
/*NÃO PODE ESCREVER ASSIM, POIS O ORDER BY PROJETA COM BASE NO SELECT E ELE NÃO ENTENDE QUE NOME É A COLUNA 1*/

SELECT NOME FROM ALUNOS
ORDER BY NUMERO, NOME;

/*NESSE CASO DEU CERTO*/
...
/*O delimitador padrão do MySQL, é o ponto e virgula ;*/
...
/*No comando acima, ao não colocar o ponto e virgula no final da query, o banco só executa, depois do ponto e virgula*/

STATUS

/*Status não é um comando SQL e sim de infra estrutura, logo, não precisa de ponto e virgula*/
...
Uptime:                 6 days 2 hours 10 min 15 sec

/*Ao fazer o comando STATUS, no campo using delimiter, é mostrado qual delimitador estamos usando*/

DELIMITER $

/* Podemos mudar nosso delimitador, como o comando acima. Mudar o delimitador é usado para criação de procedures. No caso, nosso delimitador agora é o simbolo de Dolar $*/
...
TCP port:               3306
Binary data as:         Hexadecimal
Uptime:                 6 days 2 hours 11 min 37 sec

/* Agora nosso delimitador é o dolar ao dar o status $*/

SELECT * FROM V_RELATORIO;

/*Ao tentar fazer o comando acima, ele não termina, somente se usarmos o simbolo $ no final*/
...
/*Cuidado com espaços. O banco irá contar espaço como delimitador*/

/* STORED PROCEDURES */

SELECT 'MAFRA';

/*Exemplo de programação, ou procedures. Veio uma requisição de que toda vez que a pessoa logar no banco, deve aparecer o nome da empresa*/

DELIMITER $

/*A primeira coisa, é mudar seu delimitador*/
...
/*Acima, temos um exemplo de criar uma procedure como bloco de programação Ele começa com a mudança do delimitador e fecha com o delimitador criado*/
...
/*Ao chamar uma procedure, logo após, devemos mudar o delimitador para o padrão*/

CALL NOME_EMPRESA();

/*Agora sim podemos chamar ela com o delimitador padrão*/
...
/*Exemplo de Procedure para colocar parametros*/
...
/*No MySQL, o valor de Auto_increment na primary Key ao colocar o valor é NULL*/ 

SELECT * FROM CURSOS;

/*Vamos fazer uma procedure de cadastro de novo curso para adicionar na tabela cursos*/
...
CREATE PROCEDURE CON_CURSOS(IN NOMECURSO VARCHAR(30))
BEGIN
	
	SELECT NOME,HORAS,VALOR FROM CURSOS WHERE NOME = NOMECURSO;
...
/* Lembrete! Funçoes de agregação AVG, SUM, MAX, MIN E COUNT(*), TRABALHAM COM COLUNAS*/
...
/*A Função Count(), tbm é uma função de agregação, porem, ELA CONTA OS REGISTROS, E NÃO OS VALORES*/
...
/* Se fazermos a query acima, irá estar errado! Devemos utilizar a sub-query*/
...
/* Quem vendeu menos em março, foi a Maria, e não o Carlos*/
...
/*Fazendo uma sub-query acima. Dividimos em duas querys. A primeira irá filtrar o minimo de marco (Inner query. Sempre será resolvida primeiro), e a segunda ira filtrar o nome da pessoa.*/

SELECT NOME, MARCO FROM VENDEDORES
WHERE MARCO = (SELECT MAX(FEVEREIRO) FROM VENDEDORES);

/*Fazendo uma sub-query acima. Dividimos em duas querys. A primeira irá filtrar o maximo de fevereiro (Inner query. Sempre será resolvida primeiro), e a segunda ira filtrar o nome da pessoa.*/

SELECT NOME, MARCO FROM VENDEDORES
WHERE MARCO > (SELECT AVG(MARCO) FROM VENDEDORES);

/*Fazendo uma sub-query acima. Dividimos em duas querys. A primeira irá filtrar a media de fevereiro (Inner query. Sempre será resolvida primeiro), e a segunda ira filtrar o nome da pessoa que está acima da media de fevereiro.*/
...
/*Vamos supor que agora você quer o total de vendas de cada vendedor. A operação deve ser feita em linha!!!!*/
...
/*Linhas são trabalhadas com operação aritimetica*/
...
/*Não tem como colocar auto-increment na Primary Key quando ela é adicionada*/
...
ALTER TABLE TABELA
 ...
 /*OBS: Você não consegue mudar uma coluna de Char e varchar para INT e Float*/
...
/*Quando criamos uma tabela, o banco automaticamente, cria as constraint. No caso acima, é times_ibfk_1 ao fazer o comando SHOW CREATE TABLE TIMES; 
Para melhorar isso, criamos as constraints por fora*/
...
/*Depois de criado as tabelas e seus ID`s, iremos criar por fora as chaves e as constraints*/

/* Pq fazer dessa forma? Pois primeiro, ciramos as tabelas, e depois verificamos a dependencia entre elas! Ver o video do curso MySQL Numero 72*/

ALTER TABLE CLIENTE ADD CONSTRAINT PK_CLIENTE
PRIMARY KEY(IDCLIENTE);

/*PK_CLIENTE É DE PRIMARY KEY CLIENTE*/

ALTER TABLE TELEFONE ADD CONSTRAINT FK_CLIENTE_TELEFONE
FOREIGN KEY(ID_CLIENTE) REFERENCES CLIENTE(IDCLIENTE);

/*FK é de Foreign Key. O nome é das tabela que a mesma depende, no caso cliente e o nome dela, que é telefone*/

/*Estamos nomeando as constraints criando por fora da tabela, da maneira correta. Decidimos que a tabela Telefone depende das constraints da Cliente*/

SHOW CREATE TABLE TELEFONE;

/* DICIONARIO DE DADOS */

/*Passo a passo para ver a tabela de constrainst de todo o banco*/
...
SELECT CONSTRAINT_SCHEMA AS "BANCO",
	   ...
	   /*Vendo as constraints das nossas tabelas*/
...
/*VER DIAGRAMA ENTIDADE E RELACIONAMENTO DESENHADO. Exercicio Oficina*/
...
/*ID_CARRO É UNIQUE NA TABELA CLIENTE, POIS UM CLIENTE SÓ PODE TER UM CARRO*/ 
...
/*ID_CLIENTE NÃO É UNIQUE NA TABELA TELEFONE, POIS UM CLIENTE PODE TER VARIOS TELEFONES E NÃO É NOT NULL, POIS NÃO É OBRIGATORIO NO CADASTRO*/ 
...
/*MARCA É UNIQUE, POIS NÃO PODEMOS TER A MARCA CADASTRADA DUAS VEZES NA MESMA TABELA*/
...
/*ID_MARCA NÃO É UNIQUE, POIS A TABELA CARRO PODE TER VARIAS MARCAS IGUAIS*/
...
/*COR É UNIQUE, POIS NÃO PODEMOS TER MESMA COR CADASTRADA DUAS VEZES NA MESMA TABELA*/
...
/*DEVE SER CRIADO A TABELA CARRO_COR, OU SEJA, TABELA ASSOCIATIVA. GERALMENTE, NESSAS TABELAS, SÓ TEMOS AS ID`S DE UMA TABELA E DA OUTRA*/ 
/*NESSE CASO, A PK É ASSOCIADA UMA NA OUTRA. NESSE CASO, PK (ID DE UMA TABELA, ID DA OUTRA TABELA)*/ 
...
/*ESSAS DUAS QUERYS ACIMA, SÃO REFERENTES AS ENTIDADES ASSOCIATIVA (CARRO-COR). DEVE CRIAR DUAS CONSTRAINTS FK_COR E FK_CARRO*/
...
/*Acima é mostrado uma cola de um esqueleto de uma trigger*/
/*Primeiro: definimos o delimitador*/
/*Segundo: Criamos a trigger com create trigger e colocamos um nome para a trigger*/
/*Terceiro: Definimos se a trigger será acionada Antes ou depois de um Insert ou Delete ou Update na tabela*/
/*Quarto: Para cada linha. Padrão do comando*/
/*Quinto: Definimos nosso comando entre begin e end*/
...
/*CRIAMOS UMA TABELA USUARIO, E CRIAMOS UMA TABELA DE BACK_UP DE USUARIO*/
/*SE O USUARIO FOR DELETADO, A TRIGGER IRÁ COLOCAR O ANTIGO (OLD) ID DO USUARIO NESSA TABELA BACK_UP*/
...
/*ATENCAO! NÃO É POSSIVEL CRIAR DUAS TRIGGERS NO MEMSO MOMENTO E ACAO. EX: CRIAR DUAS TRIGGER PARA ANTES DE UM DELETE*/
/*CRIAMOS UMA TRIGGER PARA BACK_UP, COM O NOME BACKUP_USER*/
/*ANTES DE DELETAR OS DADOS DA TABELA USUARIO, SERÁ FEITO A TRIGGER*/
/*O OLD, É REFERENTE AO ANTIGO VALOR DA TABELA USUARIO QUE FOI DELETADO E QUE ESSE VALOR, SERA INSERINDO ATRAVES DA TRIGGER NA TABELA BKP_USUARIO*/
...
/* A 41 - COMUNICACAO ENTRE BANCOS - CIRANDO UMA TRIGGER PARA FAZER BACKUP DE CADA VALOR INSERIDO EM UMA TABELA */
...
/*ESSA SERA A TABELA QUE IRA ARMAZENAR TODO O BACKUP DA TABELA PRODUTO*/
...
/*TODA VEZ QUE OCORRER UM UPDATE NA TABELA PRODUTO,SERA GERADA UMA TRIGGER QUE IRA MOSTRAR O USUARIO E QUE HORAS E DATA ELE FEZ*/
...
/*Cursores são programações dentro de procedures*/
...
/*Nessa operação acima, criamos duas querys para dar o total de vendas dos meses de janeiro, fevereiro e marco e outra para fazer a media*/
/*Isso deixa o banco de dados lento*/
/*Para automatizar essa query e não precisar ficar digitando, iremos criar o cursor, que é a programação dentro de uma procedure*/
...
/*Primeiro, criamos uma tabela que irá inserir os dados da operação do cursos*/
...
/*Primeiro, criamos a procedure como de costume*/
/*Declare FIM Int Default 0, é o comeco do Loop. Mais explicações abaixo*/
/*Depois, declaramos as variaveis VAR1, VAR2, VAR3, VTOTAL e VMEDIA que serão as variaveis relacionadas a tabela venda total para armazenamento*/
/*O VNOME é declarado separado do VAR1 e etc pois ele é varchar*/
/*Logo depois, declaramos o cursor que é o vetor. Não é preciso declarar o tamanho do cursor. O Cursor, seria um VETOR! Ele vai ser usado para armazenar o resultado de uma query. Nesse caso, será a query do SELECT NOME, JAN, FEV, MAR FROM VENDEDORES*/
/* Graficamente, ele ficaria [(Nome,Jan,Fev,Mar),(Nome,Jan,Fev,Mar)....]*/
/*Depois, ira vir o declare continue handler for not found set FIM = 1. Isso seria um loop. É uma variavel de manipulacao continua. Ele vai fazer o procedimento até ver que todos os elementos do vetor acabou. O loop irá comecar com 0, como delcarado acima e irá terminar e a variavel fim será 1*/
/*Open Reg pegar o resultado da query que está no cursor, no caso a query select, e realizar as operações a seguir com ela*/
/*Repeat é o Loop*/
/*FETCH é me traga o proximo do cursor e coloque no VNOME, VAR1, VAR2 e VAR3, como o select que está dentro do cursor*/
/*O IF é, se a variavel FIM não é verdadeiro, fazer a operação a seguir*/
/* Detalhe da operação*/
/*No final, inserir os resultados na tabela VEND_TOTAL*/
/*Fecha o if*/
/*Until FIM END REPET é faca ate o fim do repeat*/
/*Feche o cursor*/
/*Para mais exemplos de procedures com cursores, ver o livro MySQL de Vinicius Carvalho da Casa do Código, pagina 97 e 98*/
